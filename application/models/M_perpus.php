...
class M_perpus extends CI_Model
{
  ...
  public function kosongkan_data($table){
	...
	public function kode_otomatis(){
		...
		$this->db->select('right(id_pinjam,2) as kode', false);
		...
		$kodejadi='PJ'.$kodemax.$tgl;  //format kode;
		return $kodejadi;
	}

	
	function data($tabel, $number,$offset){
		return $query = $this->db->get($tabel,$number,$offset) ->result();		
	}
	
	//Ini model buat ambil jumlah data buku pagination
