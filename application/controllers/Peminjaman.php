...
class Peminjaman extends CI_Controller{
  ...
  //one to many
  	public function tambah_pinjam($id){
		$id = base64_decode($id);//decode nilai $id yang di kirimkan
  		if($this->session->userdata('status') != "login"){
  			$alert=$this->session->set_flashdata('alert', 'Anda belum Login');
  			redirect(base_url());
  		}else{
  			...
  			if($o>0) {
  				$this->session->set_flashdata('alert','Buku ini sudah ada dikeranjang, silahkan pilih yang lain');
  ...
  public function selesai_booking($where){
		$d = $this->M_perpus->find($where, 'peminjaman');
		$isi = array(
			'id_pinjam' => $this->M_perpus->kode_otomatis(),
      ...
      'status_pengembalian' => 'Belum Kembali'
		...
		$this->M_perpus->insert_data($isi, 'detail_pinjam');
